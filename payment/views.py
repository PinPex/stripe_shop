...
from .services import get_stripe_keys, create_checkout_session_for_item, create_checkout_session_for_order
...
def buy_item(request, id):
    if request.method == 'GET':
        try:
            session_id = create_checkout_session_for_item(id)
...
def buy_order(request, id):
    if request.method == 'GET':
        try:
            session_id = create_checkout_session_for_order(id)
            return JsonResponse({'id': session_id})
        except Exception as e:
            return JsonResponse({'error': str(e)}, status=400)
